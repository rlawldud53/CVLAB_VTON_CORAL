...
from image_datasets.dc_dataset_pose_2 import DressCodeTestDataset
from paser_helper import parse_args
from src.flux.train_utils_pose import  prepare_fill_with_mask, prepare_latents, encode_images_to_latents
...
def _tensor_to_pil_rgb(x):
    # x: (C,H,W), [-1,1] 또는 [0,1] 가정
    if isinstance(x, torch.Tensor):
        x = x.detach().cpu()
        if x.ndim == 3 and x.shape[0] in (1,3):
            pass
        elif x.ndim == 4:
            x = x[0]  # (B,C,H,W)일 경우 첫 장
        else:
            raise ValueError("pose_image tensor shape must be (C,H,W) or (B,C,H,W)")
        if x.max() <= 1.0 and x.min() >= -1.0:
            x = (x + 1.0) / 2.0  # [-1,1] -> [0,1]
        x = (x.clamp(0,1) * 255).byte()
        if x.shape[0] == 1:
            x = x.repeat(3,1,1)
        np_img = x.permute(1,2,0).numpy()  # HWC
        return Image.fromarray(np_img, mode="RGB")
    elif isinstance(x, Image.Image):
        return x.convert("RGB")
    else:
        raise TypeError("Unsupported type for pose image")

def _overlay_pil(base_rgb: Image.Image, overlay_rgb: Image.Image, alpha: float = 0.4) -> Image.Image:
    ov = overlay_rgb.resize(base_rgb.size, Image.NEAREST).convert("RGBA")
    gray = ov.convert("L")
    a = gray.point(lambda v: int(alpha*255) if v > 0 else 0)
    ov.putalpha(a)
...
def log_validation(
    ...
    import os
    save_dir = os.path.join(args.output_dir, f"{tag}_images",f"{steps:04d}")  # 예: ./output/validation_images
    ...
    with autocast_ctx:
        ...
        num_to_select = min(max_val_batches, num_batches)

        selected_indices = set(random.sample(range(num_batches), num_to_select))
        ...
        for idx, batch in enumerate(dataloader):
            ...
            gen_mask = batch['agnostic-mask'].squeeze(0).squeeze(0).to(torch.bool)
            ref_mask =  batch['agnostic-mask'].squeeze(0).squeeze(0).to(torch.bool) #batch['cloth-mask'].squeeze(0).squeeze(0).to(torch.bool)
            pose_image = batch['pose_image']

            category = batch['category']

            prompt = ["" 
                f"The pair of images highlights a clothing and its styling on a model, high resolution, 4K, 8K; "
                f"[IMAGE1] Detailed product shot of a {category} clothing"
                f"[IMAGE2] The same {category} cloth is worn by a model in a lifestyle setting."
                ] * len(batch['image'])
            ...
            fname = f"{tag}_{steps:04d}_{global_idx:06d}.png"
            ...
            result = pipeline(
                ...
                guidance_scale=30,
                img_name=f"{global_idx:06d}",
                run_name=f"{tag}_{steps:04d}",
                gen_mask=gen_mask,
                ref_mask=ref_mask,
            ...
            for j, img in enumerate(result):
                ...
                fname_base = f"{tag}_{steps:04d}_{global_idx:06d}"
                out_path    = os.path.join(save_dir, f"{fname_base}.png")
                out_path_ov = os.path.join(save_dir, f"{fname_base}_overlay.png")
                ...
                try:
                    ...
                    # pose to PIL
                    pose_pil = _tensor_to_pil_rgb(pose_image[j])
                    right_ov = _overlay_pil(right, pose_pil, alpha=0.4)

                    merged = Image.new("RGB", (w, h))
                    merged.paste(left,  (0,   0))
                    merged.paste(right_ov, (mid, 0))

                    merged.save(out_path_ov)

                except Exception as e:
                    print(f"[WARN] overlay-right-panel failed at idx {global_idx}: {e}")
...
def is_rank0(accelerator):
    return getattr(accelerator.state, "process_index", accelerator.process_index) == 0
...
def main(args):
    ...
    # config = FluxTransformer2DModel.load_config(args.pretrained_model_name_or_path, subfolder="transformer")
    # transformer = FluxTransformer2DModel.from_config(config)
    # print(args.pretrained_inpaint_model_name_or_path)
    ...
    train_verification_dataset = DressCodeTestDataset(
        dataroot_path=args.dataroot,
        phase="train",
        order="paired",
        size=(args.height, args.width),
    ...
    if accelerator.is_main_process:
        tracker_name = args.project_name #"dreambooth-flux-inpaint"
    ...
    if args.resume_from_checkpoint:
        ...
        if path is None:
            ...
            resume_opt_steps_in_epoch = global_step % num_update_steps_per_epoch

            if resume_opt_steps_in_epoch > 0:
                skip_batches = resume_opt_steps_in_epoch * args.gradient_accumulation_steps
                train_dataloader = accelerator.skip_first_batches(train_dataloader, skip_batches)

                if accelerator.is_main_process:
                    logger.info(
                        f"[Resume] Skipping {skip_batches} mini-batches "
                        f"({resume_opt_steps_in_epoch} optimizer steps) within current epoch."
                    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                garment_image = batch["cloth_pure"].to(dtype=vae.dtype)
                pose_image = batch["pose_image"].to(dtype=vae.dtype)
                # garment_image_0_1 = (batch["cloth_pure"] + 1.0) / 2
                garment_image = garment_image.to(dtype=vae.dtype)


                #if accelerator.local_process_index==0 : breakpoint()
                # pause(accelerator)
                #flow_GT = batch["flow_GT"].to(dtype=vae.dtype)
                ...
                pose_input = encode_images_to_latents(vae, pose_image, weight_dtype, args.height, args.width)
                ...
                # pose_image_ids = prepare_latents(
                #             vae_scale_factor,
                #             batch_size,
                #             args.height,
                #             args.width * 2,
                ...
                # pose_image_ids = pose_image_ids.reshape(64, 48 * 2, -1)
                # pose_image_ids = pose_image_ids[:, 48:,]
                latent_w, latent_h = (args.width // 16), (args.height // 16)
                latent_image_ids = latent_image_ids.reshape(latent_h, latent_w * 2 , -1)
                pose_image_ids = latent_image_ids[:,latent_w:,:]
                ...
                packed_pose_input = FluxFillPipeline._pack_latents(
                    pose_input,
                    batch_size=pose_input.shape[0],
                    num_channels_latents=pose_input.shape[1],
                    height=pose_input.shape[2],
                    width=pose_input.shape[3],
                )

                packed_noisy_model_input = packed_noisy_model_input.reshape(packed_noisy_model_input.shape[0], latent_h, latent_w * 2, -1)
                packed_pose_input = packed_pose_input.reshape(packed_pose_input.shape[0], latent_h, latent_w, -1)

                packed_noisy_model_input = torch.cat([packed_noisy_model_input, packed_pose_input], dim=2) 
                packed_noisy_model_input = packed_noisy_model_input.reshape(packed_noisy_model_input.shape[0], packed_noisy_model_input.shape[1] * packed_noisy_model_input.shape[2], -1 )
                ...
                ######check#####
                B, S, D = packed_noisy_model_input.shape
                assert transformer.x_embedder.in_features == D, (
                    f"x_embedder.in_features={transformer.x_embedder.in_features} vs input D={D}"
                )
                ################
                ...
                model_pred = model_pred.reshape(model_pred.shape[0], latent_h, latent_w*3, -1)
                model_pred = model_pred[:,:,:latent_w*2,:]
                model_pred = model_pred.reshape(model_pred.shape[0], model_pred.shape[1] * model_pred.shape[2], -1)
                #model_pred = model_pred[:,:latent_image_ids.shape[0]*latent_image_ids.shape[1],:]
                ...
                # flow_loss_sum = 0
                # layer_cnt = 0
                # assert len(flow_loss.keys()) == 57, f"number of losses in flow loss must be 57!"
                # for layer, loss in flow_loss_dict.items():
                #     if loss is not None:
                #         assert len(loss) == 1
                #         flow_loss_sum += loss[0]
                #         layer_cnt += 1
                #     else:
                #         raise NotImplementedError(f"Flow loss for layer {layer} not implemented.")
                    
                # flow_loss = flow_loss_sum / layer_cnt
                ...
                assert target.shape == model_pred.shape
            ...
            if accelerator.sync_gradients:
                ...
                do_validate = (
                                    ...
                                    and is_rank0(accelerator)  # STRICT rank-0 check
