...
def prepare_fill_with_mask(
    ...
    image_batch_size = image.shape[0] #[1,3,1024,1536]
    ...
    if isinstance(mask, torch.Tensor):
        ...
        mask = mask_processor.preprocess(mask, height=height, width=width) #[1,1,1024,1536]
    ...
    # Encode to latents
    image_latents = vae.encode(masked_image.to(vae.dtype)).latent_dist.sample() #[1,16,128,192]
    image_latents = (
        image_latents - vae.config.shift_factor
    ) * vae.config.scaling_factor
    image_latents = image_latents.to(dtype) #[1,16,128,192]

    # Process mask following the example's specific rearrangement
    mask = mask[:, 0, :, :] if mask.shape[1] > 1 else mask[:, 0, :, :]
    mask = mask.to(torch.bfloat16) #[1,1024,1536]
    ...
    image_cond = torch.cat([image_latents, mask], dim=-1) # [1,6144,384]
...
def prepare_latents(
    ...
    # print("Adjusted height and width for latents:", height, width)
